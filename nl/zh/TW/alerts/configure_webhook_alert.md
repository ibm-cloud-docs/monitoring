...
# 使用 {{site.data.keyword.monitoringshort}} API 配置 Webhook 警示
{: #configure_webhook_alert}

若要就度量值配置 Webhook 警示，請定義規則及 Webhook 通知。然後，向 {{site.data.keyword.monitoringshort}} 服務登錄規則及通知方法。
{:shortdesc}
...
* 使用 Webhook 警示的通知範本。如需相關資訊，請參閱[通知範本](/docs/services/cloud-monitoring?topic=cloud-monitoring-config_alerts_ov#notification_template)。
* 在本端目錄建立檔案，例如 `~/cloud-monitoring/notifications`。

例如，使用 vi 編輯器建立檔案 **webhook.json**： 
