...
在 {{site.data.keyword.Bluemix}} 中供应 {{site.data.keyword.mon_full_notm}} 服务的实例，并为度量源配置了 Sysdig 代理程序后，可以通过该服务的 Web UI 来查看、监视和管理数据。
...
{: caption="表 3. 面板类型" caption-side="top"} 
...
{: caption="表 4. 面板任务" caption-side="top"} 
...
{: caption="表1. 可在 Web UI 中运行的仪表板任务" caption-side="top"} 
...
{: caption="表 2. 用于以编程方式管理仪表板的任务" caption-side="top"} 
...
可以解决触发事件的情境。在等待触发事件的条件再次运行并将其状态设置为*正常*时，可以将该事件设置为**已解决**，从而以图形方式显示该问题已解决。
{: #tip}
