...
| `HTTP Top Requests`  | Utilizza questo dashboard per monitorare gli URL più richiesti del tuo server web. Ad esempio, il numero totale di richieste, i tempi massimi e medi per servire le richieste e la quantità di traffico contenuta nelle richieste e nelle risposte. |   |
...
**Nota:** ognuno di questi dashboard mostra linee tratteggiate e caselle grigie per le istanze che non hanno un agent Sysdig e per cui è stata rilevata la comunicazione.
