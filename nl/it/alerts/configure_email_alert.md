...
# Configurazione di un avviso email utilizzando l'API {{site.data.keyword.monitoringshort}}
{: #configure_email_alert}

Per configurare un avviso email per una metrica, definisci una regola e una notifica email. Quindi, registra il metodo di notifica e la regola con il servizio {{site.data.keyword.monitoringshort}}.
{:shortdesc}
...
## Passo 3: crea una notifica email
{: #step3}


Per creare un file di notifica che invia un'email, considera le seguenti informazioni:

* Utilizza un template di notifica per l'invio di email. Per ulteriori informazioni, vedi [Template di notifica](/docs/services/cloud-monitoring/config_alerts_ov.html#notification_template).
* Crea il file in una directory locale, ad esempio, `~/cloud-monitoring/notifications`.

Ad esempio, crea il file **email.json** utilizzando l'editor vi: 
