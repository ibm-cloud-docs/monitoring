...
# {{site.data.keyword.monitoringshort}} API を使用した E メール・アラートの構成
{: #configure_email_alert}

メトリックに E メールのアラートを構成するには、ルールと E メールによる通知を定義します。 次に、
...
E メールを送信する通知ファイルを作成するには、次の情報を考慮します。

* E メールを送信するための通知テンプレートを使用します。 詳しくは、
...
例えば、以下のように、vi エディターを使用してファイル **email.json** を作成します。 
