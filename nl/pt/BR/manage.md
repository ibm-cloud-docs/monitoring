...
| [Criar um grupo ](/docs/services/Monitoring-with-Sysdig?topic=Sysdig-groups#create_group)            | Criar um grupo. |
...
A **Agregação** de dados ocorre automaticamente quando você configura um gráfico ou cria um alerta para uma métrica. 

Há diferentes tipos de agregação que são usados para métricas: 
* A agregação de tempo é sempre executada antes da agregação de grupo.
* Para criar comparações de várias séries e vários alertas, também é possível dividir dados
agregados em seções menores, as quais são chamadas de **segmentos**, usando rótulos. 

A agregação de tempo é sempre executada antes da agregação de grupo.

A Sysdig agrega dados ao longo do tempo. Em seguida, pegue esses pontos de dados e agrupe-os para
exibir os dados em métricas, painéis ou calcular os limites de alerta.
{: tip}
...
Em gráficos de séries temporais que incluem dados para cinco minutos ou menos, 
* Os pontos de dados são desenhados na resolução de 10 segundos
* A agregação de tempo não ocorre.

A agregação de tempo ocorre automaticamente em qualquer das situações a seguir:

* Em gráficos de séries temporais que incluem dados para uma quantia de tempo maior que cinco
minutos, os pontos de dados são desenhados como um agregado para um intervalo de tempo apropriado. Por
exemplo, para um gráfico que mostra dados para uma hora, cada ponto de dados representa um intervalo de
um minuto.
* Quando você exibe dados históricos, os dados são acumulados ao longo do tempo. É possível escolher quais pontos de dados utilizar ao visualizar dados mais antigos.
...
| `rate (timeAvg)`   | Valor médio da métrica no período avaliado.            |
| `maximum`	         | O valor mais alto durante o período avaliado.                          |
| `minimum`         | O valor mais baixo durante o período avaliado.                           |
| `sum`              | A soma combinada da métrica no período avaliado.             |
...
A taxa e a média são tipos de agregação semelhantes. Elas geralmente fornecem o mesmo resultado. No entanto, o cálculo de cada uma é diferente. Se a agregação de tempo for configurada como um minuto, o agente Sysdig será configurado para recuperar seis amostras, uma a cada 10 segundos. Observe que, em alguns casos, as amostras podem não estar lá devido a desconexões ou outras circunstâncias.
...
| `maximum`	         | Valor mais alto das amostras do intervalo.           |
| `minimum`	         | Valor mais baixo das amostras do intervalo.            |
...
**A agregação de grupos depende da segmentação.** Para uma visualização que mostra as métricas para um grupo de itens, se a seleção *Segmentar por* for mudada para dividir os itens individuais, a agregação de grupo não ocorrerá.
...
Um administrador do Sysdig pode definir qualquer número de equipes. Para cada equipe, um administrador
pode configurar as informações a seguir:
* `A equipe padrão`: é possível configurar essa para ser a equipe na qual qualquer usuário que faz login na IU da web pela primeira vez. 
* `O ponto de entrada padrão`: é possível especificar a visualização na IU da web
que é aberta toda vez que um usuário efetua login. Os pontos de entrada válidos são a visualização *Explorar*, a visualização *Painéis*, a visualização *Eventos*,
a visualização *Alertas* e a visualização *Configurações*.
...
* `Permissões`: é possível ativar ou desativar os recursos a seguir: *Capturas
do Sysdig* e *eventos de infraestrutura*. Os arquivos de captura estão visíveis para
os membros da equipe. **Nota:** as capturas incluem informações detalhadas de cada contêiner em um host, independentemente do escopo da equipe. Quando os eventos de infraestrutura estão ativados, os usuários
podem visualizar todos os eventos de infraestrutura customizados de cada usuário e agente Sysdig na instância.

Por padrão, uma equipe **Monitorar operações** é predefinida para cada instância
do {{site.data.keyword.mon_full_notm}}.
...
** Nota: ** 
* Um administrador deve alternar para a equipe *Monitorar operações* para criar equipes
ou mudar as configurações para outras equipes.
...
Um administrador pode executar qualquer uma das ações a seguir para restringir as permissões de
...
| [Excluindo uma equipe](/docs/services/Monitoring-with-Sysdig?topic=Sysdig-teams#teams_delete)      | Exclua uma equipe. </br>**Nota:** quando você excluir uma equipe, os usuários que pertencerem somente a essa equipe serão movidos para a equipe padrão. |
